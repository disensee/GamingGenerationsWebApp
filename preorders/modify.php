...
if(isset($_POST['id']))
{
    $phone = preg_replace('/(\W*)/', '', $_POST['s_phone']);
    $amount = number_format($_POST['s_deposit'],2);
    
    $sql = "UPDATE orders SET order_cust_name = '".$_POST['s_cname']."', order_phone = '".$phone."', order_email = '".$_POST['s_cemail']."', game_id = '".$_POST['s_game']."', order_amount = '".$amount."', employee = '".$_POST['s_employee']."', order_console = '".$_POST['s_console']."'
            WHERE order_id = ".$_POST['id'];
        
    /*$sql = "INSERT INTO orders (order_amount, order_phone, order_email, game_id, order_amount, order_store, order_timestamp, employee, order_console) VALUES
            ('".$_POST['s_cname']."', '".$phone."', '".$_POST['s_cemail']."', ".$_POST['s_game'].", '".$amount."', '".$x_store."', '".time()."', '".$_POST['s_employee']."', '".$_POST['s_console']."');";
    */
    $db->query($sql);
    
    //doEmail($db->insert_id());
    
    header("Location: index.php?cphone=".$phone);
}
?>
...
<ul>
<li><label for="Employee">Employee</label><input class="itext" type="text" name="s_employee" id="Employee" value="<?=$data['employee'];?>" /></li>
<li class="iheader">Customer Info</li>
<li><label for="ContactName">Contact Name</label><input class="itext" type="text" name="s_cname" id="ContactName" value="<?=$data['order_cust_name'];?>" /></li>
<li><label for="ContactEmail">Contact Email</label><input class="itext" type="text" name="s_cemail" id="ContactEmail" value="<?=$data['order_email'];?>" /></li>
<li><label for="ContactPhone">Contact Phone</label><input class="itext" type="text" name="s_phone" id="ContactPhone" value="<?=$data['order_phone'];?>" /></li>
<li class="iheader">Preorder Information</li>
...
{
?>
<option value="<?=$row['game_id'];?>" <?=($row['game_id'] == $data['game_id'] ? 'selected="selected"' : '');?>><?=$row['game_name'];?> ($<?=$row['game_price'];?>)</option>
<?
}
?>
</select>
</li>
<li><label for="Console">Console</label>
<select class="iselect" name="s_console" id="Console">
<?
$consoles = array('PS3', 'Wii', 'Xbox 360', 'PSP', "DS", "PS4", "Xbox One");
sort($consoles);
for($i=0;$i<count($consoles);$i++)
{
    echo '<option value="'.$consoles[$i].'" '.($consoles[$i] == $data['order_console'] ? 'selected="selected"' : '').'>'.$consoles[$i].'</option>';
}
?>
</select>
</li>
<li><label for="Deposit">Deposit Amount</label><input class="itext" type="text" name="s_deposit" id="Deposit" value="<?=$data['order_amount'];?>" onblur="if(this.value < 5) { alert('The customer must put a minimum of $5 down towards the pre-order.'); setTimeout(setFocus, 100); }" /></li>

<li><label>&nbsp;</label><input type="hidden" name="id" value="<?=$data['order_id'];?>" /><input type="submit" class="ibutton" name="Submit" id="Submit" value="Update!" /></li>
